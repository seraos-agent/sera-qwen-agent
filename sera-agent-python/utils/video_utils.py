...
async def _submit_video_job(payload: dict) -> str:
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(VIDEO_URL, headers=HEADERS, json=payload, timeout=30.0)
...
async def _poll_video_job(task_id: str) -> str:
    ...
    for attempt in range(1, MAX_POLLS + 1):
        ...
        async def do_poll():
            async with httpx.AsyncClient() as client:
                resp = await client.get(poll_url, headers=poll_headers, timeout=30.0)
...
async def generate_video_with_happyhorse_t2v(
    ...
    """
    duration = min(duration, MAX_DURATION)  # hard cap 8 seconds
